function [caster] = caster(X, Xaxis, Yaxis, Zaxis)
%%Calculate caster angle for suspension defined by X and tireRadius
...
for i = 1:L(1)
    for j = 1:L(2)
        for k = 1:L(3)
            caster(i,j,k) = vectsAngle(projectVectPlane(X(13:15,i,j,k)-X(16:18,i,j,k),[0 1 0]),[0 0 1],'deg');
        end
    end
end
end
