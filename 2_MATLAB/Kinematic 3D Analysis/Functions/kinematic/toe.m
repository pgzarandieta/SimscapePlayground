function [toe] = toe(X, Xaxis, Yaxis, Zaxis)
%%Calculate toe angle for suspension defined by X. 
% Toe is positive when turning to the right
...
for i = 1:L(1)
    for j = 1:L(2)
        for k = 1:L(3)
            toe(i,j,k) = vectsAngle(projectVectPlane(X(28:30,i,j,k),[0 0 1]),[0 1 0],'deg')*sign(X(28,i,j,k));
        end
    end
end
end
